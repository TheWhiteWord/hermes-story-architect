...
exports.parse = exports.screenplayProperties = exports.StructToken = exports.Location = exports.lexer = exports.titlePageDisplay = exports.regex = void 0;
...
function lexer(s, type, replacer, titlepage = false) {
    if (!s) {
        return s;
    }
    ...
    if (titlepage) {
        s = s.replace(exports.regex.link, replacer.link);
    }
    s = s.replace(exports.regex.note_inline, replacer.note).replace(/\\\*/g, '[star]').replace(/\\_/g, '[underline]').replace(/\n/g, replacer.line_break);
    // if (regex.emphasis.test(s)) {                         // this was causing only every other occurence of an emphasis syntax to be parsed
    while (i--) {
        style = styles[i];
        match = exports.regex[style];
        if (match.test(s)) {
...
exports.StructToken = StructToken;
class screenplayProperties {
}
exports.screenplayProperties = screenplayProperties;
var parse = function (original_script, cfg, generate_html) {
    var lastFountainEditor;
    var config = (0, configloader_1.getFountainConfig)(lastFountainEditor);
    var emptytitlepage = true;
    var script = original_script, result = {
        title_page: {
            tl: [],
            tc: [],
            tr: [],
    ...
    var lines_length = lines.length, current = 0, scene_number = 1, current_depth = 0, match, text, last_title_page_token, thistoken, last_was_separator = false, 
    //top_or_separated = false,
    token_category = "none", last_character_index, dual_right, state = "normal", previousCharacter, cache_state_for_comment, nested_comments = 0, title_page_started = false;
    ...
    const latestSectionOrScene = (depth, condition) => {
        try {
            if (depth <= 0) {
                return null;
            }
            else if (depth == 1) {
                var lastItem = (0, utils_1.last)(result.properties.structure.filter(condition));
                return lastItem;
            }
            else {
                var prevSection = latestSectionOrScene(depth - 1, condition);
                if (prevSection.children != null) {
                    var lastChild = (0, utils_1.last)(prevSection.children.filter(condition));
                    if (lastChild)
                        return lastChild;
                }
    ...
    const processInlineNote = (text, linenumber) => {
        ...
        if (match = text.match(new RegExp(exports.regex.note_inline))) {
            ...
            if (level) {
                ...
                for (let i = 0; i < match.length; i++) {
                    match[i] = match[i].slice(2, match[i].length - 2);
                    result.properties.structure.push({ text: match[i], id: '/' + linenumber, isnote: true, children: [], level: 0, notes: [], range: new vscode_1.Range(new vscode_1.Position(linenumber, 0), new vscode_1.Position(linenumber, match[i].length + 4)), section: false, synopses: [] });
                    irrelevantTextLength += match[i].length + 4;
                }
            }
        }
    ...
    const processDialogueBlock = (token) => {
        let textWithoutNotes = token.text.replace(exports.regex.note_inline, "");
        processInlineNote(token.text, token.line);
        token.time = (0, utils_1.calculateDialogueDuration)(textWithoutNotes);
        if (!cfg.print_notes) {
    ...
    for (var i = 0; i < lines_length; i++) {
        ...
        thistoken = (0, token_1.create_token)(text, current, i, new_line_length);
        ...
        if (text.trim().length === 0 && text !== "  ") {
            ...
            if (state == "dialogue")
                pushToken((0, token_1.create_token)(undefined, undefined, undefined, undefined, "dialogue_end"));
            if (state == "dual_dialogue")
                pushToken((0, token_1.create_token)(undefined, undefined, undefined, undefined, "dual_dialogue_end"));
            state = "normal";
        ...
        if (!title_page_started && exports.regex.title_page.test(thistoken.text)) {
            state = "title_page";
        }
        if (state === "title_page") {
            if (exports.regex.title_page.test(thistoken.text)) {
                ...
                let keyformat = exports.titlePageDisplay[thistoken.type];
                if (keyformat) {
        ...
        const latestSection = (depth) => latestSectionOrScene(depth, token => token.section);
        if (state === "normal") {
            if (thistoken.text.match(exports.regex.line_break)) {
                token_category = "none";
            }
            else if (result.properties.firstTokenLine == Infinity) {
                result.properties.firstTokenLine = thistoken.line;
            }
            let sceneHeadingMatch = thistoken.text.match(exports.regex.scene_heading);
            if (sceneHeadingMatch) {
                thistoken.text = thistoken.text.replace(/^\./, "");
                if (cfg.each_scene_on_new_page && scene_number !== 1) {
                    var page_break = (0, token_1.create_token)();
                ...
                if (match = thistoken.text.match(exports.regex.scene_number)) {
                    thistoken.text = thistoken.text.replace(exports.regex.scene_number, "");
                    thistoken.number = match[1];
                }
                let cobj = new StructToken();
                cobj.text = thistoken.text;
                cobj.children = null;
                cobj.range = new vscode_1.Range(new vscode_1.Position(thistoken.line, 0), new vscode_1.Position(thistoken.line, thistoken.text.length));
                if (current_depth == 0) {
                ...
                const location = (0, utils_1.parseLocationInformation)(sceneHeadingMatch);
                if (location) {
                    const locationSlug = (0, utils_1.slugify)(location.name);
                    if (result.properties.locations.has(locationSlug)) {
                        ...
                        if (values.findIndex(it => it.scene_number == scene_number) == -1) {
                            values.push(Object.assign({ scene_number: scene_number, line: thistoken.line }, location));
                        }
                        result.properties.locations.set(locationSlug, values);
                    }
                    else {
                        result.properties.locations.set(locationSlug, [Object.assign({ scene_number, line: thistoken.line }, location)]);
                    }
                }
                scene_number++;
            }
            ...
            else if (thistoken.text.match(exports.regex.centered)) {
                thistoken.type = "centered";
                thistoken.text = thistoken.text.replace(/>|</g, "").trim();
            }
            else if (thistoken.text.match(exports.regex.transition)) {
                thistoken.text = thistoken.text.replace(/> ?/, "");
                thistoken.type = "transition";
            }
            else if (match = thistoken.text.match(exports.regex.synopsis)) {
            ...
            else if (match = thistoken.text.match(exports.regex.section)) {
            ...
            else if (thistoken.text.match(exports.regex.page_break)) {
                thistoken.text = "";
                thistoken.type = "page_break";
            }
            else if (thistoken.text.match(exports.regex.character) && i != lines_length && i != lines_length - 1 && ((lines[i + 1].trim().length == 0) ? (lines[i + 1] == "  ") : true)) {
                ...
                if (config.print_dialogue_numbers)
                    (0, Decorations_1.AddDialogueNumberDecoration)(thistoken);
                thistoken.text = (0, utils_1.trimCharacterForceSymbol)(thistoken.text);
                if (thistoken.text[thistoken.text.length - 1] === "^") {
                    if (cfg.use_dual_dialogue) {
                        ...
                        pushToken((0, token_1.create_token)(undefined, undefined, undefined, undefined, "dialogue_begin"));
                    }
                    thistoken.text = thistoken.text.replace(/\^$/, "");
                }
                else {
                ...
                let character = (0, utils_1.trimCharacterExtension)(thistoken.text).trim();
            ...
            if (thistoken.text.match(exports.regex.parenthetical)) {
    ...
    if (generate_html) {
        ...
        if (!emptytitlepage) {
            for (const section of Object.keys(result.title_page)) {
                result.title_page[section].sort(helpers_1.default.sort_index);
                ...
                while (current_index < result.title_page[section].length) {
                    var current_token = result.title_page[section][current_index];
                    ...
                    switch (current_token.type) {
                        case 'title':
                            titlehtml.push(`<h1 class="haseditorline titlepagetoken" id="sourceline_${current_token.line}">${current_token.html}</h1>`);
                            break;
                        case 'header':
                            header = current_token;
                            break;
                        case 'footer':
                            footer = current_token;
                            break;
                        default:
                            titlehtml.push(`<p class="${current_token.type} haseditorline titlepagetoken" id="sourceline_${current_token.line}">${current_token.html}</p>`);
                            break;
                    }
                    current_index++;
                }
            ...
            if ((current_token.type == "action" || current_token.type == "centered") && !current_token.ignore) {
                ...
                switch (current_token.type) {
                    case 'scene_heading':
                        ...
                        break;
                    case 'transition':
                        html.push('<h2 class="haseditorline" id="sourceline_' + current_token.line + '">' + current_token.text + '</h2>');
                        break;
                    case 'dual_dialogue_begin':
                        html.push('<div class=\"dual-dialogue\">');
                        break;
                    case 'dialogue_begin':
                        html.push('<div class=\"dialogue' + (current_token.dual ? ' ' + current_token.dual : '') + '\">');
                        break;
                    case 'character':
                        ...
                        break;
                    case 'parenthetical':
                        html.push('<p class="haseditorline parenthetical\" id="sourceline_' + current_token.line + '" >' + current_token.html + '</p>');
                        break;
                    case 'dialogue':
                        if (current_token.text == "  ")
                            html.push('<br>');
                    ...
                    case 'dual_dialogue_end':
                        ...
                        break;
                    case 'section':
                        html.push('<p class="haseditorline section" id="sourceline_' + current_token.line + '" data-position=\"' + current_token.line + '\" data-depth=\"' + current_token.level + '\">' + current_token.text + '</p>');
                        break;
                    case 'synopsis':
                        html.push('<p class="haseditorline synopsis" id="sourceline_' + current_token.line + '" >' + current_token.html + '</p>');
                        break;
                    case 'lyric':
                        html.push('<p class="haseditorline lyric" id="sourceline_' + current_token.line + '">' + current_token.html + '</p>');
                        break;
                    case 'note':
                        html.push('<p class="haseditorline note" id="sourceline_' + current_token.line + '">' + current_token.html + '</p>');
                        break;
                    case 'boneyard_begin':
                        html.push('<!-- ');
                        break;
...
exports.parse = parse;
//# sourceMappingURL=afterwriting-parser.js.map